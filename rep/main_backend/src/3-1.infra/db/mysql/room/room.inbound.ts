...
import { DB_ROOMS_ATTRIBUTE_NAME, DB_TABLE_NAME, DB_USERS_ATTRIBUTE_NAME, MYSQL_DB } from "../../db.constants";
...
export class SelectRoomIdFromMysql extends SelectDataFromDb<Pool> {
  ...
  async select({ attributeName, attributeValue, }: { attributeName: string; attributeValue: string; }): Promise<GetRoomInfoDbResult | undefined> {
